...
### Added
- **Behavior Specs** — new step in Phase 2 (`document.md` Step 4) for creating developer-ready behavior specs at `specs/behavior/REQ-*.md`. Structured Fields + Scenarios tables per requirement. Living document — add detail incrementally. Added to `conventions.md` directory structure and frontmatter schema
- **`generate-test-cases.py` script** — deterministic test case generator that reads `specs/behavior/REQ-*.md`, parses Scenarios tables, produces XLSX with 3 tabs (UAT, SIT, E2E). Same input = same output. No LLM cost. Falls back to CSV if openpyxl not installed. Available as phase-independent command via "generate test cases"
- **Test Cases tab in dashboard + report** — shows behavior spec coverage per REQ: fields count, UAT/SIT/E2E scenario counts, status. Summary metrics at top. Dashboard is tab 16, report is sheet 16. Updated `sync-project-data.py`, `update-project-data.py`, `dashboard-server.py`, `generate-report.py`
