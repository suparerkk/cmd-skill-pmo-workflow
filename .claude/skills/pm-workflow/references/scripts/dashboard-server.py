...
<div class="tp" id="tab-traceability"><table><thead><tr><th>REQ IDs</th><th>PRD</th><th>Epic</th><th>Story</th><th>Task</th><th>Task Name</th><th>Status</th></tr></thead><tbody id="trt"></tbody></table></div>
...
function render(d){
  ...
  document.getElementById('trt').innerHTML=(d.traceability||[]).length?d.traceability.map(t=>`<tr><td>${t.reqs||'—'}</td><td>${t.prd||'—'}</td><td>${t.epic||'—'}</td><td>${t.story||'—'}</td><td>${t.task_id||'—'}</td><td>${t.task_name||'—'}</td><td>${bd(t.status)}</td></tr>`).join(''):'<tr><td colspan="7" style="text-align:center;color:var(--steel)">No traceability data yet</td></tr>';
