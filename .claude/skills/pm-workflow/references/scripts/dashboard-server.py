...
<div class="tp" id="tab-traceability"><table><thead><tr><th>REQ ID</th><th>Title</th><th>PRD</th><th>Behavior Spec</th><th>Persona</th><th>Epic</th><th>Stories</th><th>Tasks</th><th>Done</th></tr></thead><tbody id="trt"></tbody></table></div>
...
<script>
let D={};
function switchTab(tab){document.querySelectorAll('.nav button').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.tp').forEach(p=>p.classList.remove('active'));const btn=document.querySelector(`.nav button[data-tab="${tab}"]`);if(btn){btn.classList.add('active');document.getElementById('tab-'+tab).classList.add('active');history.replaceState(null,null,'#'+tab)}}
document.getElementById('nav').addEventListener('click',e=>{if(e.target.tagName!=='BUTTON')return;switchTab(e.target.dataset.tab)});
(function(){const h=location.hash.replace('#','');if(h)switchTab(h)})();
...
function render(d){
  ...
  ftbl('tt',d.tasks||[],['id','name','status','story','depends_on','effort','updated'],'tc','ts','tf','status');
  ...
  document.getElementById('trt').innerHTML=(d.traceability||[]).length?d.traceability.map(t=>`<tr><td>${t.req_id||'—'}</td><td>${t.req_title||'—'}</td><td>${t.prd?'<span class="bd done">Yes</span>':'<span class="bd draft">—</span>'}</td><td>${t.behavior_spec||'<span class="bd draft">—</span>'}</td><td>${t.persona||'—'}</td><td>${t.epic||'—'}</td><td>${t.stories||'—'}</td><td>${t.tasks||0}</td><td>${t.tasks>0?(t.tasks_done+'/'+t.tasks):'—'}</td></tr>`).join(''):'<tr><td colspan="9" style="text-align:center;color:var(--steel)">No requirements yet</td></tr>';
